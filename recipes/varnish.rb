...
unless File.exist?(File.join(node[:magento][:dir], '/app/code/community/Phoenix/VarnishCache/etc/default_3.0.vcl'))
  ...
  execute "Copying Community Varnish Configuration" do
    user 'root'
    command "cp -f #{File.join(node[:magento][:dir], '/app/code/community/Phoenix/VarnishCache/etc/default_3.0.vcl')} /etc/varnish/default.vcl"
    notifies :restart, 'service[varnish]'
  end
...
if node[:rax][:magento][:single]
  if !File.exist?(File.join(node[:magento][:dir], "app/etc/local.xml")) && node[:rax][:magento][:varnish][:use_varnish]
    # Configuration for PageCache module to be enabled
    execute "pagecache-database-inserts" do
      command "/usr/bin/mysql #{node[:magento][:db][:database]} -u #{node[:magento][:db][:username]} -h #{node[:magento][:db][:host]} -P #{node[:mysql][:port]} -p#{node[:magento][:db][:password]} < /root/pagecache_inserts.sql"
      action :nothing
    end
